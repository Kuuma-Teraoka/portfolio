...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    // /0と/を追加するので+2
    char inputWavPath[nullNum(path_name, 0) + nullNum(argv[1], 0)+2] ;
    sumStrings(path_name, argv[1], inputWavPath) ;

    char modify[] = "modified.wav" ;
    char modifiedWavPath[nullNum(path_name, 0) + 14] ;
    sumStrings(path_name, modify, modifiedWavPath) ;

    // fopenのための"sound01"のような文字列を作り格納するための配列。
    char sound[13] = "00 sound.wav" ;
    char soundPath[nullNum(path_name, 0) + 14] ;



    if((fileWav = fopen(inputWavPath, "r")) == 0)
    {
